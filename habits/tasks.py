...
def send_telegram_notification():
    ...
    for habit in usefull_habits_list:
        ...
        message_main = (
            f'{
                habit.user.first_name} вам необходимо выполнить привычку - {
                habit.action} в {formatted_date}' f'\nЕё нужно выполнить за {
                habit.time_required} в {
                    habit.place}.' f'\nПосле вы получите вознаграждение - {
                        habit.reward} либо {
                            habit.related_habit}.\n')

        if habit.frequency == 'day' and habit.date <= current_time:
            habit.date = datetime.now(
                pytz.timezone(time_zone)) + timedelta(days=1)
            formatted_next_date = habit.date.strftime('%Y-%m-%d %H:%M')
            message_extended = f'\nДалее ваша привычка должна быть выполнена завтра до {
                formatted_next_date}.'
            ...
            habit.date = datetime.now(
                pytz.timezone(time_zone)) + timedelta(days=7)
            formatted_next_date = habit.date.strftime('%Y-%m-%d %H:%M')
            message_extended = f'\nДалее ваша привычка должна быть выполнена в течении недели до {
                formatted_next_date}.'
            message = message_main + message_extended
            tg_bot.send_message(chat_id=habit.user.telegram, text=message)
            habit.save()
